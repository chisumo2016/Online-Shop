...
it('can add a product to the cart', function (ProductWasAddedToCart $event){
    //create a projector
    expect($this->projector)->toBeInstanceOf(class: CartProjector::class);

    //build event dd($event);
    $cart = Cart::query()->with(['items.purchasable'])->find($event->cartID);
    expect($cart->items->count())->toEqual(0);
    expect($cart->total)->toEqual(0);

    //trigger method -> passing in the build event
    ...
    //test the custom
    $cart->refresh();
    expect($cart->items->count())->toEqual(1);
    expect($cart->total)->toEqual($cart->items->first()->purchasable->retail);
        //$cart->items->count())->toEqual(1);
